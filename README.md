...
(`image`, `second`, `request`, or `1m_tokens`); `amount` is the exact unit price, or use `min`/`max`
instead when the price depends on request parameters (resolution, duration, quality) and a single
number would mislead. `amount` and `min`/`max` are mutually exclusive, and an optional `note` is a
short qualifier shown with the price. The SDK does not price or bill anything: pricing is
declaration metadata for catalog surfaces, and actual charges always come from the provider.
